...
def manhattanSegmentalDist(x, y, Ds):
    """ Compute the Manhattan Segmental Distance between 
        x and y considering the dimensions on Ds."""
    dist = 0
    for d in Ds:
        dist += np.abs(x[d] - y[d])
    return dist / len(Ds)
...
""" Run PROCLUS on a database to obtain a set of clusters and 
        ...
        - seed:    seed
"""
def projectedClustering(X, k = 2, l = 3, minDeviation = 0.1, A = 30, B = 3, niters = 30, seed = 1234):
    ...
    Mcurr = np.random.permutation(M)[0:k] 
    Mbest = None
    ...
    # smallest Manhattan segmental distance of m_i to all (k-1) other medoids with respect to D_i
    deltais = np.zeros(k)
    for i in range(k):
        minDist = np.inf
        for j in range(k):
